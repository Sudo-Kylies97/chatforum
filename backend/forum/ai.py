...
def analyse_post(post):
    ...
    try:
        ...
        post.ai_status = Post.AIStatus.COMPLETE
        post.ai_model = model
...
def analyse_thread_vibe(post):
    if not enabled("AI_VIBE_ENABLED"):
        Post.objects.filter(pk=post.pk).update(vibe_status=Post.AIStatus.DISABLED)
        return
    comments = list(post.comments.order_by("created_at").values_list("body", flat=True))
    if not comments:
        Post.objects.filter(pk=post.pk).update(
            vibe=Post.Vibe.UNKNOWN, vibe_status=Post.AIStatus.COMPLETE
        )
        return
    prompt = (
        "Classify the overall vibe of these forum comments. Return JSON only with vibe exactly one of Toxic, Constructive, Humorous, Informative.\nComments:\n"
        + "\n---\n".join(comments[-50:])
    )
    try:
        result = client().chat.completions.create(
        ...
        value = str(json.loads(result.choices[0].message.content)["vibe"]).title()
        mapped = {
            "Toxic": "toxic",
        ...
        Post.objects.filter(pk=post.pk).update(vibe=mapped, vibe_status=Post.AIStatus.COMPLETE)
    except Exception:
        Post.objects.filter(pk=post.pk).update(vibe_status=Post.AIStatus.FAILED)
