...
class ForumAPITests(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(
            author=self.alex, title="A useful topic", body="Enough body content"
        )
    ...
    def test_anonymous_can_read_but_not_write(self):
        ...
        self.assertIn(
            self.client.post("/api/v1/posts/", {"title": "No", "body": "Anonymous"}).status_code,
            (401, 403),
        )
    ...
    def test_authenticated_user_can_create_immutable_post(self):
        ...
        response = self.client.post(
            "/api/v1/posts/", {"title": "Created", "body": "A new contribution"}
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["ai_status"], Post.AIStatus.DISABLED)
        self.assertEqual(
            self.client.patch(
                f"/api/v1/posts/{response.data['id']}/", {"title": "Edited"}
            ).status_code,
            405,
        )
    ...
    def test_comment_and_moderation_permissions(self):
        ...
        self.assertTrue(self.post.is_misleading)

    def test_private_ai_review_is_hidden_from_regular_users(self):
        self.post.ai_moderation_score = 0.91
        self.post.ai_moderation_rationale = "Needs review"
        self.post.ai_needs_review = True
        self.post.save()
        self.client.force_authenticate(self.sam)
        ...
        self.assertEqual(moderator["ai_moderation_score"], 0.91)
    ...
    def test_personal_token_is_returned_once_and_can_be_revoked(self):
        ...
        external = APIClient()
        external.credentials(HTTP_AUTHORIZATION=f"Bearer {raw}")
...
class AIServiceTests(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(
            author=self.user, title="Chip update", body="A factual claim"
        )

    @patch.dict(
        os.environ,
        {"AI_API_KEY": "test", "AI_MODERATION_ENABLED": "true", "AI_MODERATION_THRESHOLD": "0.75"},
    )
    ...
    def test_structured_analysis_sets_category_and_private_preflag(self, mock_client):
        ...
        choice = SimpleNamespace(
            message=SimpleNamespace(
                content='{"misinformation_confidence":0.9,"rationale":"Check this claim"}'
            )
        )
        mock_client.return_value.chat.completions.create.return_value = SimpleNamespace(
            choices=[choice]
        )
        analyse_post(self.post)
        self.post.refresh_from_db()
        self.assertTrue(self.post.ai_needs_review)
        self.assertFalse(self.post.is_misleading)
    ...
    def test_thread_vibe_uses_comment_context(self, mock_client):
        ...
        Comment.objects.create(
            post=self.post, author=self.user, body="This is a useful and clear explanation."
        )
        choice = SimpleNamespace(message=SimpleNamespace(content='{"vibe":"Constructive"}'))
        mock_client.return_value.chat.completions.create.return_value = SimpleNamespace(
            choices=[choice]
        )
        analyse_thread_vibe(self.post)
        self.post.refresh_from_db()
...
class AuthenticationTests(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(
            author=self.other, title="Searchable", body="Clean energy storage"
        )
        self.client = APIClient()
    ...
    def test_password_login_and_logout(self):
        ...
        good = self.client.post(
            "/api/v1/auth/login/", {"username": "reader", "password": "password123"}
        )
        self.assertEqual(good.status_code, 200)
        self.assertEqual(good.data["username"], "reader")
