...
def post_queryset(request):
    likes = (
        Like.objects.filter(post=OuterRef("pk"), user=request.user.pk)
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def like(self, request, pk=None):
        ...
        try:
            ...
            created = False
        if not created:
            return Response({"detail": "You already liked this post."}, status=409)
    ...
    def moderation(self, request, pk=None):
        ...
        post = self.get_object()
        post.is_misleading = bool(request.data.get("is_misleading"))
        post.flagged_by = request.user if post.is_misleading else None
        post.flagged_at = timezone.now() if post.is_misleading else None
        post.ai_needs_review = False
...
class TokenViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return PersonalAPIToken.objects.filter(user=self.request.user).order_by("-created_at")
