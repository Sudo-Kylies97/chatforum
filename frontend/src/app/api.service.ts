...
export class ApiService {
  ...
  posts(category='', search=''):Observable<Page<Post>|Post[]> { const url=search?`${this.base}/posts/search/?q=${encodeURIComponent(search)}`:`${this.base}/posts/${category?`?category=${category}`:''}`; return this.http.get<Page<Post>|Post[]>(url,{withCredentials:true}); }
