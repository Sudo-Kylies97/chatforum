...
it('sets feed filters deterministically',()=>{store.filter('technology');expect(store.selectedCategory()).toBe('technology');});
...
it('turns connection failures into user-facing state',()=>{api.posts.and.returnValue(throwError(()=>new HttpErrorResponse({status:503,error:{detail:'Service unavailable.'}})));store.loadPosts();expect(store.loading()).toBeFalse();expect(store.error()).toContain('Service unavailable');});
